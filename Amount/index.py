...
class AccountDetailsApp(MDApp):
    ...
    def on_start(self):
        ...
        #self.load_colors()
        add_entry = self.screen_manager.get_screen('add_entry')
        add_entry.ids.transaction_date.bind(on_release=lambda x: add_entry.show_date_picker())
        add_entry.ids.transaction_type.bind(on_release=lambda x: add_entry.transaction_type_menu.open())
        add_entry.ids.transaction_method.bind(on_release=lambda x: add_entry.transaction_method_menu.open())
    ...
    def build_xlsx(self, xlsx_file):
        ...
        head_format = workbook.add_format({'bold':True, 'border':True, 'align':'center', 'valign':'center', 'font':'Bell MT', 'font_size':12})
        body_format = workbook.add_format({'border':True, 'align':'left', 'valign':'center', 'font':'Aptos', 'font_size':10})
    ...
    def save_xlsx_to_folder(self, folder_uri, filename, file_path):
        """
        Robust SAF write: create or overwrite a file inside the picked tree/folder.
        """
        current_activity = PythonActivity.mActivity
        resolver = current_activity.getContentResolver()

        try:
            # STEP 1: get tree document id and children URI
            tree_doc_id = DocumentsContract.getTreeDocumentId(folder_uri)
            print("tree_doc_id:", tree_doc_id)  # debug

            children_uri = DocumentsContract.buildChildDocumentsUriUsingTree(
                folder_uri, tree_doc_id
            )
        except JavaException as e:
            print("Error getting tree/document id or building children uri:", e)
            return

        # STEP 2: search for existing file in the folder
        existing_file_uri = None
        cursor = None
        try:
            cursor = resolver.query(children_uri,
                                    ["display_name", "document_id"],
                                    None, None, None)
            if cursor:
                while cursor.moveToNext():
                    # read display name and document id safely
                    try:
                        display_index = cursor.getColumnIndex("display_name")
                        docid_index = cursor.getColumnIndex("document_id")
                        name = cursor.getString(display_index) if display_index >= 0 else None
                    except JavaException:
                        # some providers may use different column names — skip if reading fails
                        name = None

                    if name == filename:
                        # found existing file -> build document URI for overwrite
                        doc_id = cursor.getString(docid_index)
        ...
        # STEP 3: Create or reuse URI
        try:
            if existing_file_uri:
                out_uri = existing_file_uri
                print("Overwriting existing:", out_uri.toString())
            else:
                # createDocument returns a Uri or null on failure
                ...
                if out_uri:
                    ...
                    print("createDocument returned null - cannot create file")
                    return
        except JavaException as e:
            print("Error creating document:", e)
            return

        # STEP 4: Write bytes to the out stream
        try:
    ...
    def export_data(self):
        '''if platform == "android":
            xlsx_file = 'data.xlsx'
            xlsx_path = f'{os.getcwd()}/{xlsx_file}'
            self.build_xlsx(xlsx_path)
            ...
            while True:
                ...
                if count == 1:
                    ...
                    xlsx_file.replace(f'{count-1}', count)'''
...
if platform == "android":
    def on_activity_result(requestCode, resultCode, intent):
        app = AccountDetailsApp.get_running_app()
        if app:
